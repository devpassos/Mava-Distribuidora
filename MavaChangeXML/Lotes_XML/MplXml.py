...
def imprimeProdutosXML(listaObj):
    ...
    for det in listaObj:
        ...
        # Passa a lista de objetos da tag prod para a variável prod
        prod = det[0]
...
def incluirLoteXML(xml_name):
    ...
    nome_Fornec = root[0][0][1].find(
        '{http://www.portalfiscal.inf.br/nfe}xFant').text
    ...
    for x in root[0][0]:
        if x.tag == '{http://www.portalfiscal.inf.br/nfe}det':
            ...
            qtCompra = x[0].find(
                '{http://www.portalfiscal.inf.br/nfe}qCom').text
