...
LPAD(TRUNC((SUM(PEDI.QT) / PROD.QTEMBMASTER)),3,' ')||' - '||RPAD(prod.unidademaster,4,' ') AS QT_TOTAL_CAIXAS,

--(TRUNC((SUM(PEDI.QT) / PROD.QTEMBMASTER)) * EMB.QTEMBALAGEM)) / EMB.QTEMBALAGEM) AS QT_ITENS_AVULSO, --/ (emb.qtembalagem

LPAD(trunc(mod((Sum(PEDI.QT)), PROD.QTEMBMASTER)/ emb.qtembalagem),3,' ')||' - '||RPAD(prod.embalagem,10,' ') AS QT_ITENS_AVULSO,
